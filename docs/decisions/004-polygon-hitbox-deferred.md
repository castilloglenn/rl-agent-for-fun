# 004: Polygon hitbox, after the refactor (roadmap step 3c)

**Date:** 2026-09-25. **Status:** Accepted, scheduled for roadmap step 3c (moved up from the multi-car step).
...
## Update: moved to step 3c

Crash = game over (step 3e) makes the growing box unacceptable: a diagonal car would crash while visibly pixels away from the border, which is unfair to human drivers and teaches agents the wrong margins. So the polygon lands in step 3c, before crashes:

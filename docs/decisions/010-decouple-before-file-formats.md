...
| 6 | The driver is a label string | Can't tell which agent and checkpoint drove a replay | A **driver record** (type, id, checkpoint or device) | 4d |
| 7 | The reward function and code version aren't in run configs | Runs couldn't be compared or reproduced exactly | `config.json` records the reward function, game-defining config, and code version | 4g |

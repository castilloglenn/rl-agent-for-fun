...
**Date:** 2026-09-25. **Status:** Accepted. Implemented so far: the player in driver records (4d), the driver interface and the 12 canonical actions in code (4f, `src/drivers/`), with a test that all 32 key combinations behave like their canonical action. Per-player recordings with kept runs (4i, `src/replay/recordings.py`). The agent driver, `agent:<id>` in the registry, with the record `{"type": "agent", "id", "checkpoint"}` (5a1, `src/agents/`). Planned: training (5a2 onward), 5b. Baseline drivers use the record `{"type": "baseline", "id": ...}`.
